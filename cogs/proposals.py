...
class ProposalsCog(commands.Cog, guild_ids=GUILD_IDS):
    ...
    async def process_proposals(self) -> None:
        ...
        logger.info("[TASK] Running Proposals Processing...")
        ...
        for data in proposals:
            try:
                ...
                upvotes = sum(1 for reaction in message.reactions if reaction.emoji == "👍") - 1
                downvotes = sum(1 for reaction in message.reactions if reaction.emoji == "👎") - 1

                embed = discord.Embed(description="Voting has ended for this proposal.", colour=0xFF0000)
        ...
        logger.info(f"[TASK] Proccessed {len(proposals)} Proposals.")
